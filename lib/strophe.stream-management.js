	...
	strophe_js.Strophe.addConnectionPlugin('streamManagement', {
		...
		*/
		...
		_interceptDoDisconnect: function(condition) {
			if (this.getResumeToken()
				...
				strophe_js.Strophe.debug('SM stored resume state, handler count: ' + this._resumeState.handlers.length);
			}
		...
		statusChanged: function (status) {
			if (!this.getResumeToken()
				...
				strophe_js.Strophe.debug('SM reset state');
		...
		*/
		...
		_handleResumeFailed: function(elem) {
			this._c._changeConnectStatus(strophe_js.Strophe.Status.ERROR, null, elem);
		...
		_handleResumed: function(elem) {
			...
			if (this.logging && this._unacknowledgedStanzas.length > 0) {
				strophe_js.Strophe.debug('SM Sending unacknowledged stanzas', this._unacknowledgedStanzas);
				for(const stanza of this._unacknowledgedStanzas) {
					this._c.send(stanza);
				}
			} else {
				strophe_js.Strophe.debug('SM No unacknowledged stanzas', this._unacknowledgedStanzas);
			}
